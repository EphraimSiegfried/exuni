...
def cli():
    ...
    parser.add_argument(
        "-d",
        "--database-path",
        help="Path to the database (if it does not exist, it will be created)",
        default="dpi.db",
    )
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update the database from Google Sheets"
    )
    update_parser.add_argument(
        "type",
        choices=["group", "contribution"],
        help="Specify whether to update the group sheet or the contribution sheet.",
    )
    update_parser.add_argument(
        "sheet_key", type=str, help="Google Sheets key to update the database."
    )
    update_parser.add_argument(
        "--credentials",
        help="Path to the Google Sheets API credentials file.",
        default="credentials.json",
        type=lambda p: (
            p if Path(p).is_file() else parser.error(f"File {p} does not exist.")
        ),
    )
    ...
    query_parser.add_argument(
        "query_type",
        choices=["less_than", "later_than", "without_feedback"],
    ...
    query_parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Parameters for the query (e.g., for less_than: score value; for later_than: date).",
    )
...
def main():
    ...
    if args.command == "update":
        ...
        if args.query_type == "less_than" and args.query_params:
            ...
            result = queries.get_feedbacks_where_score_less_than(
                score, exercise_num, conn
            )
        elif args.query_type == "later_than" and args.query_params:
            date = args.query_params[0]
            exercise_num = int(args.query_params[1])
            result = queries.get_students_submitted_feedback_later_than(
                date, exercise_num, conn
            )
        elif args.query_type == "without_feedback" and args.query_params:
            exercise_num = int(args.query_params[0])
            result = queries.get_students_without_feedback_submission(
                exercise_num, conn
            )
        for row in result:
            row = map(str, row)
